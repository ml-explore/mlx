...
class SVM {
 ...
 public:
  ...
  mx::array loss(const mx::array& X, const mx::array& Y) {
    ...
    auto hinge =
        mx::maximum(mx::subtract(mx::array(1.0f), margins), mx::array(0.0f));
    auto hinge_loss = mx::mean(hinge);
    auto regularization = mx::multiply(
        mx::array(lambda_) / mx::array(2.0f), mx::sum(mx::square(weights_)));
  ...
  mx::array predict(const mx::array& X) const {
    auto scores = mx::add(mx::matmul(X, weights_), bias_);
    return mx::where(
        mx::greater_equal(scores, mx::array(0.0f)),
        mx::array(1.0f),
        mx::array(-1.0f));
  };
  ...
  void fit(const mx::array& X, const mx::array& Y, int epochs) {
    auto loss_fn =
        [this, &X, &Y](const std::vector<mx::array>& params) -> mx::array {
      ...
      auto hinge_loss = mx::mean(hinge);
      auto regularization =
          mx::multiply(mx::array(lambda_ / 2.0f), mx::sum(mx::square(weights)));

      return mx::add(hinge_loss, regularization);
    };

    auto value_and_grad_fn =
        mx::value_and_grad(loss_fn, std::vector<int>{0, 1});

    for (int epoch = 0; epoch < epochs; ++epoch) {
      std::vector<mx::array> params = {weights_, bias_};
      ...
      weights_ =
          mx::subtract(weights_, mx::multiply(mx::array(lr_), grad_weights));

      bias_ = mx::subtract(bias_, mx::multiply(mx::array(lr_), grad_bias));

      // Evaluate computation graph

      mx::eval(weights_, bias_, loss_value);

      if (epoch % 100 == 0) {
        std::cout << "Epoch: " << epoch
                  << " | Loss: " << loss_value.item<float>() << std::endl;
      }
    }
  };
  ...
  float accuracy(const mx::array& X, const mx::array& Y) const {
    auto predictions = predict(X);

    auto correct = mx::equal(predictions, Y);

    auto accuracy = mx::mean(mx::astype(correct, mx::float32));
