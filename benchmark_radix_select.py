...
def verify_correctness(b, v, k, dtype=mx.float32):
    ...
    for i in range(b):
        ...
        assert np.all(partitioned_values[:k] <= kth_value), f"Row {i}: elements before k are not all <= kth"
        
        # All elements after k should be >= kth_value
        assert np.all(partitioned_values[k+1:] >= kth_value), f"Row {i}: elements after k are not all >= kth"
...
def main():
    ...
    for dtype, dtype_name in dtypes:
        print(f"\nDtype: {dtype_name}")
        print(f"{'Config':<25} {'ArgPartition':<15} {'Partition':<15} {'Sort':<15} {'Speedup':<10}")
        print("-" * 80)
        ...
        for b, v, k in configs:
            try:
                ...
                print(f"{config_str:<25} {argpart_ms:>12.3f}ms {part_ms:>12.3f}ms {sort_ms:>12.3f}ms {speedup:>8.2f}x")
