        ...
        * ``D``: The per-head dimension.
        ...
        Args:
            ...
            mask (str or array, optional): The mask to apply to the
               ...
               alignment (last query aligns with last key), equivalent to PyTorch's
               ``causal_lower_right``. When ``T_q > T_kv``, it uses upper-left
               alignment, equivalent to PyTorch's ``is_causal=True``.
