...
class TestFastSDPA(mlx_tests.MLXTestCase):
    ...
    def test_sdpa_full_d96_v64(self):
        ...
        with mlx_tests.scoped_env(MLX_SDPA_NAX_D96_V64="1"):
            for dtype, (B, qH, kH, qL, kL) in product(
                (mx.float16, mx.bfloat16, mx.float32), cells
            ):
                if (
                    dtype == mx.float32
                    and os.environ.get("MLX_ENABLE_TF32", "1") == "0"
                ):
                    continue
                q = (
                ...
                k = (0.5 * mx.random.normal((B, kH, kL + 32, 96))).astype(dtype)[
                    :, :, :kL
                ]
                masks = [
                    None,
                ...
                for dv, mask, with_sinks in product((64, 96), masks, (False, True)):
                    with self.subTest(
                        ...
                        v = (0.5 * mx.random.normal((B, kH, kL + 32, dv))).astype(
                            dtype
                        )[:, :, :kL]
                        if qL == 9:
                            # V sliced from the interleaved K/V projection.
                            v = (0.5 * mx.random.normal((B, kL, kH, 2 * dv))).astype(
                                dtype
                            )
                            v = v.transpose(0, 2, 1, 3)[..., dv:]
                        sinks = (
                            mx.random.normal((qH,)).astype(dtype)
                            if with_sinks
                            else None
                        )
                        ref = mlx_ref_attn(
    ...
    @unittest.skipIf(not mx.metal.is_available(), "Metal kernel path only")
    def test_sdpa_d96_v64_full_fallback(self):
        if mx.default_device() != mx.gpu:
            self.skipTest("requires GPU")
        q = mx.random.normal((1, 4, 9, 96)).astype(mx.float16)
        k = mx.random.normal((1, 4, 33, 96)).astype(mx.float16)
        v = mx.random.normal((1, 4, 33, 64)).astype(mx.float16)
        scale = 96**-0.5
        with mlx_tests.scoped_env(MLX_SDPA_NAX_D96_V64="0"):
            with self.assertRaisesRegex(ValueError, "NAX kernel is disabled"):
        ...
        with mlx_tests.scoped_env(MLX_SDPA_NAX_D96_V64="1"):
            bad_v = mx.random.normal((1, 4, 33, 80)).astype(mx.float16)
            with self.assertRaisesRegex(ValueError, "supports head dims"):
                mx.fast.scaled_dot_product_attention(
                    q, k, bad_v, scale=scale, force_fused=True
                )
    ...
    def test_sdpa_vector_d96_v64(self):
        ...
        with mlx_tests.scoped_env(MLX_SDPA_VECTOR_D96_V64="1"):
            for dtype, (B, qH, kH, qL, kL) in product(
                ...
                for mask in masks:
                    with self.subTest(
                        ...
                        self.assertTrue(mx.allclose(ref, out, atol=tol, rtol=tol))
        with mlx_tests.scoped_env(MLX_SDPA_VECTOR_D96_V64="0"):
            with self.assertRaisesRegex(ValueError, "vector kernel is disabled"):
                mx.fast.scaled_dot_product_attention(
                    q, k, v, scale=scale, force_fused=True
                )
            out = mx.fast.scaled_dot_product_attention(q, k, v, scale=scale)
            ref = mlx_ref_attn(q, k, v, scale)
            self.assertTrue(mx.allclose(ref, out, atol=2e-2, rtol=2e-2))
