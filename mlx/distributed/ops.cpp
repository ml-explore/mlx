...
array all_reduce_opt(
  ...
  if (op == "sum" || op.empty()) {
    ...
    throw std::invalid_argument(
        "Prod operation not yet supported in optimized all_reduce");
  } else {
    throw std::invalid_argument("Unknown all_reduce operation: " + op);
  }
}
...
array reduce_scatter_opt(
  ...
  if (op == "sum" || op.empty()) {
    ...
    throw std::invalid_argument(
        "Max operation not yet supported in optimized reduce_scatter");
  } else if (op == "min") {
    // TODO: Implement min reduce-scatter
    throw std::invalid_argument(
        "Min operation not yet supported in optimized reduce_scatter");
  } else {
