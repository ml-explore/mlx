...
array fast_inference_sdpa(
  ...
  if (q.ndim() == 0 || k.ndim() == 0 || v.ndim() == 0) {
    throw std::invalid_argument(
        "[fast_inference_sdpa] Got 0 dimension input. Inputs must "
        "have at least one dimension.");
  }
  for (const auto& tensor : {q, k, v}) {
    if (tensor.ndim() != 4) {
      std::ostringstream msg;
      msg << "[fast_inference_sdpa] input with shape " << tensor.shape()
          << " expected to be rank 4";
      throw std::invalid_argument(msg.str());
    }
  }

  size_t batch_dim = q.shape(0);
  if (batch_dim != 1) {
    std::ostringstream msg;
    msg << "[fast_inference_sdpa] Expected batch dimension of 1. ";
    throw std::invalid_argument(msg.str());
  }
  for (const auto& tensor : {q, k, v}) {
    if (tensor.shape(0) != batch_dim) {
      std::ostringstream msg;
      msg << "[fast_inference_sdpa] mismatching batch dimension for input with shape "
          << tensor.shape() << ".";
      throw std::invalid_argument(msg.str());
    }
  }

  // Below shape checking assumes following tensor shapes:
  // Q ~ [B H R D], B ~ batch, H ~ n_heads, R ~ query sequence length (1), D ~
  // d_k (e.g. 128) K ~ [B H_kv L D], B ~ batch, H ~ n_kv_heads, L ~ kv sequence
  // length (cache size + 1), D ~ d_k V ~ [B H_kv L D_v], B ~ batch, H ~
  // n_kv_heads, L ~ kv sequence length (cache size + 1), D ~ d_v; Note that for
  // Multihead attention, n_heads == n_kv_heads; Multi-query attention,
  // n_kv_heads == 1; and for Grouped Query Attention, n_kv_heads < n_heads,
  // n_kv_heads > 1, n_heads % n_kv_heads == 0. K, V tensors must have same
  // number of heads.

  // Q, K must have matching last dims (d_k);
  if (q.shape(-1) != k.shape(-1)) {
    std::ostringstream msg;
    msg << "[fast_inference_sdpa] query, keys expected to have matching d_k; found query "
        << q.shape() << " for keys " << k.shape() << ".";
    throw std::invalid_argument(msg.str());
  }

  // head_dim must be 128.
  constexpr const int supported_head_dim = 128;
  if (q.shape(-1) != supported_head_dim) {
    std::ostringstream msg;
    msg << "[fast_inference_sdpa] required: args.head_dim == 128 ; found "
        << q.shape(-1) << " .";
    throw std::invalid_argument(msg.str());
  }

  // K, V must have matching head dims (n_kv_heads);
  size_t n_q_heads = q.shape(-3);
  size_t n_kv_heads = k.shape(-3);

  if (k.shape(-3) != v.shape(-3)) {
    std::ostringstream msg;
    msg << "[fast_inference_sdpa] keys, values expected to have matching n_kv_heads; found keys with n_heads "
        << k.shape(-3) << " for values with n_heads " << v.shape(-3) << ".";
    throw std::invalid_argument(msg.str());
  }
  ...
  if (n_q_heads % n_kv_heads != 0) {
    std::ostringstream msg;
    msg << "[fast_inference_sdpa] n_heads must be a multiple of n_kv_heads, found n_heads "
        << n_q_heads << " for n_kv_heads " << n_kv_heads << ".";
    throw std::invalid_argument(msg.str());
  }

  // q_seq_len == 1 for now in decoding (no mask) implementation
  if (q.shape(-2) > 1 && !m.has_value()) {
    std::ostringstream msg;
    msg << "[fast_inference_sdpa] query sequence length must be 1 at decode time, found  "
        << q.shape(-2) << ".";
    throw std::invalid_argument(msg.str());
  }

  // Type promotion: common type between q, k, v;
  auto out_type = promote_types(q.dtype(), k.dtype());
  auto out_type_kv = promote_types(k.dtype(), v.dtype());
  auto final_type = promote_types(out_type, out_type_kv);

  if (q.dtype() != out_type) {
    q = astype(q, out_type, s);
  }
  if (k.dtype() != out_type) {
    k = astype(k, out_type, s);
  }

  if (v.dtype() != final_type) {
    v = astype(v, final_type, s);
  }

  auto out_shape =
      std::vector<int>({q.shape(0), q.shape(1), q.shape(2), v.shape(-1)});

  auto prompt_encoding_with_or_without_tiling = [](const array& q_tensor,
                                                   const array& k_tensor,
                                                   const array& v_tensor,
                                                   const array& mask_tensor,
                                                   const float alpha,
                                                   const Dtype out_dtype,
                                                   StreamOrDevice s) {
    auto q_scaled = multiply(array(alpha), q_tensor, s);
    ...
    auto tile_if_needs_repeat = [](const array& arr,
      ...
      constexpr const int heads_axis =
          1; // heads axis, assumes tensors arranged  as [0, 1, 2, 3] -> [Batch,
             // Heads, Sequence, Hidden]
    ...
    auto masked_s = add(s_tensor, mask_tensor, s);
    auto p = softmax(masked_s, std::vector<int>({-1}), s);
    auto out_tensor = matmul(p, v_tensor_tiled, s);
    out_tensor = astype(out_tensor, out_dtype, s);
    return out_tensor;
  };

  if (m.has_value()) {
    auto m_value = m.value();
    return prompt_encoding_with_or_without_tiling(
        q, k, v, m_value, scale, final_type, s);
  } else {
    auto out = array(
        out_shape,
        final_type,
        std::make_unique<FastInferenceSDPA>(to_stream(s), scale, false),
        {q, k, v});
    return out;
  }
}
