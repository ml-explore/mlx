...
#pragma unroll
  ...
  unfolded.set_status(array::evaluated);
#endif
...
void gemm_conv_nd(
  ...
  std::cout << in_unfolded << std::endl;
  std::cout << wt_reshaped << std::endl;
#endif
...
void gemm_conv(
  ...
  if (conv_nd == 2) {
    ConvParams<2> params{
        ...
        {input_dilation[0], input_dilation[1]},
        groups,
        flip,
        {in.shape(1), in.shape(2)},
        {wt.shape(1), wt.shape(2)},
  ...
  throw std::runtime_error(
      fmt::format("[conv] Unsupported gemm_conv for {}D conv.", conv_nd));
}
