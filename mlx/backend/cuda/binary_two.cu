      ...
      if constexpr (cu::supports_binary_two_op<Op, CTYPE_IN, CTYPE_OUT>()) {
        ...
        if (bopt == BinaryOpType::General) {
              ...
              [&](auto large) {
                ...
                if (ndim <= 3) {
                  dispatch_1_2_3(ndim, [&](auto dims_constant) {
                    ...
                    OutType* out_a_ptr = gpu_ptr<OutType>(out_a);
                    OutType* out_b_ptr = gpu_ptr<OutType>(out_b);
                    IdxT rest_val = rest;
          ...
          dispatch_bool(out_a.data_size() > UINT32_MAX, [&](auto large) {
            ...
            OutType* out_a_ptr = gpu_ptr<OutType>(out_a);
            OutType* out_b_ptr = gpu_ptr<OutType>(out_b);
            IdxT size = out_a.data_size();
            void* params[] = {&a_ptr, &b_ptr, &out_a_ptr, &out_b_ptr, &size};
            encoder.add_kernel_node(
                reinterpret_cast<void*>(kernel),
                num_blocks,
                block_dims,
