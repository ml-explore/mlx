...
template <typename T>
__device__ __forceinline__ T absmax_val(T x) {
  if constexpr (cuda::std::is_same_v<T, bool>) {
    return x;
  } else if constexpr (cuda::std::is_unsigned_v<T>) {
    return x; // unsigned types are non-negative
...
__global__ void all_reduce(T* in, U* out, size_t block_step, size_t size) {
  ...
  for (; i + block.size() * N <= check; i += block.size() * N) {
    ...
    for (int j = 0; j < N; j++) {
      if constexpr (cuda::std::is_same_v<ReduceOp, AbsMax>) {
        accs[0] = op(accs[0], absmax_val(cast_to<U>(vals[j])));
      } else {
  ...
  if (i < check) {
    ...
    for (int i = 0; i < N; i++) {
      if constexpr (cuda::std::is_same_v<ReduceOp, AbsMax>) {
        accs[0] = op(accs[0], absmax_val(cast_to<U>(vals[i])));
      } else {
