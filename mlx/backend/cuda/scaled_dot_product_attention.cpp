...
void sdpa_backward_cudnn(
  ...
  // TODO: Handle donation.
  d_q.set_data(cu::malloc_async(d_q.nbytes(), encoder.stream()));
  d_k.set_data(cu::malloc_async(d_k.nbytes(), encoder.stream()));
  d_v.set_data(cu::malloc_async(d_v.nbytes(), encoder.stream()));
