...
void FastGruCell::eval_gpu(
  ...
  // Allocate output buffer on GPU (required before set_output_array)
  out.set_data(allocator::malloc(out.nbytes()));

  // Copy only when not row-contiguous (same as LSTM); avoids 3 copies per step
  // for normal forward path where matmul/previous step outputs are contiguous.
  ...
  constexpr uint32_t threads_per_group = 512;  // same as LSTM
