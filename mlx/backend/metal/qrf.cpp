...
void qrf_block(
  ...
  for (int jj = 0; jj < R; jj++) {
    ...
    if constexpr (!RUN_ON_CPU_FOR_DEBUG) {
      auto launch_kernel = [&](const std::string& kernel_name) {
        ...
        compute_encoder->setBytes(&jj, sizeof(jj), 3);
        compute_encoder->setBytes(&startc, sizeof(startc), 4);
        compute_encoder->setBytes(&m, sizeof(m), 5);
  ...
  if constexpr (!RUN_ON_CPU_FOR_DEBUG) {
    ...
    compute_encoder->setBytes(&m, sizeof(m), 3);
  ...
  if (startc + R < n) {
    ...
    if constexpr (!RUN_ON_CPU_FOR_DEBUG) {
      ...
      compute_encoder.set_input_array(a_rest, 3); // out
