...
void CustomKernel::eval_gpu(
  ...
  {
    // Use a hash of the kernel source as part of the library key so that
    // different source variants coexist safely in the cache without evicting
    // a library that may still be referenced by an in-flight command buffer.
    // This avoids the use-after-free race (MLX issue #3347) where
    // clear_library() destroyed a MTLComputePipelineState while a previous
    // operation still held it in an uncommitted Metal command buffer.
  }
