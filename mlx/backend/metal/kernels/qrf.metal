...
[[kernel]] void qrf_compute_sq_norm_xc(
  ...
  mlx_atomic_fetch_add_explicit(betas_data, val, jj);
}
...
[[kernel]] void qrf_compute_v(
  ...
  // The previous kernel leaves the square norm of xc in betas_data[jj];
  const float norm_xc = metal::sqrt(mlx_atomic_load_explicit(betas_data, jj));
...
[[kernel]] void qrf_compute_w_first_col(
    device const float* betas_data,
    device const float* y_data,
    device float* w_data,
    constant const int& m,
  ...
  w_data[colmajor_idx(i, 0, m)] =
      -betas_data[0] * y_data[colmajor_idx(i, 0, m)];
}
