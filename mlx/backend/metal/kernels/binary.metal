                  ...
                  instantiate_binary_all(name, uint64, uint64_t, uint64_t, op) \
                      instantiate_binary_all(name, int8, int8_t, int8_t, op)   \
                          instantiate_binary_all(                              \
                              name, int16, int16_t, int16_t, op)               \
                              instantiate_binary_all(                          \
                                  name, int32, int32_t, int32_t, op)           \
                                  instantiate_binary_all(                      \
                                      name, int64, int64_t, int64_t, op)       \
                                      instantiate_binary_all(                  \
                                          name,                                \
                                          complex64,                           \
                                          complex64_t,                         \
...
#define instantiate_binary_types_bool(name, op)                               \
  instantiate_binary_all(name, bool_, bool, bool, op) instantiate_binary_all( \
      name, uint8, uint8_t, bool, op)                                         \
              ...
              instantiate_binary_all(name, uint64, uint64_t, bool, op)        \
                  instantiate_binary_all(name, int8, int8_t, bool, op)        \
                      instantiate_binary_all(name, int16, int16_t, bool, op)  \
                          instantiate_binary_all(                             \
                              name, int32, int32_t, bool, op)                 \
                              instantiate_binary_all(                         \
                                  name, int64, int64_t, bool, op)             \
                                  instantiate_binary_all(                     \
                                      name, float16, half, bool, op)          \
                                      instantiate_binary_all(                 \
                                          name, float32, float, bool, op)     \
                                          instantiate_binary_all(             \
                                              name,                           \
                                              bfloat16,                       \
                                              bfloat16_t,                     \
...
instantiate_binary_types(add, Add) instantiate_binary_types(div, Divide)
    instantiate_binary_types_bool(eq, Equal) instantiate_binary_types_bool(
        ge,
        Greater) instantiate_binary_types_bool(geq, GreaterEqual)
        instantiate_binary_types_bool(le, Less) instantiate_binary_types_bool(
            leq,
            LessEqual) instantiate_binary_types_bool(neq, NotEqual)
    ...
    instantiate_binary_all(
        ...
        half,
        bool,
        NaNEqual) instantiate_binary_all(naneq, float32, float, bool, NaNEqual)
