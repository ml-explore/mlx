...
// LSTM: i,f,g,o = gates; cell_new = f*cell_prev + i*g; hidden_new =
// o*tanh(cell_new)
[[host_name("lstm_cell_fused_float")]] [[kernel]] void lstm_cell_fused_float(
  ...
  if (h_base + 4u <= hidden_size) {
    ...
    float4 o4 = fast_sigmoid4(
        *reinterpret_cast<const device float4*>(
            input_proj + base + 3u * hidden_size) +
        *reinterpret_cast<const device float4*>(
            hidden_proj + base + 3u * hidden_size));
    float4 c_prev4 =
        *reinterpret_cast<const device float4*>(cell_prev + prev_base);
...
// BFloat16 path
[[host_name("lstm_cell_fused_bfloat16")]] [[kernel]] void
lstm_cell_fused_bfloat16(
  ...
  if (h_base + 4u <= hidden_size) {
    ...
    float4 c_prev4 = float4(
        cell_prev[prev_base],
        cell_prev[prev_base + 1],
        cell_prev[prev_base + 2],
        cell_prev[prev_base + 3]);

    float4 c_new4 = f4 * c_prev4 + i4 * g4;
    float4 h_new4 = o4 * metal::fast::tanh(c_new4);
    output_cell[prev_base] = bfloat16_t(c_new4.x);
    output_cell[prev_base + 1] = bfloat16_t(c_new4.y);
    output_cell[prev_base + 2] = bfloat16_t(c_new4.z);
    output_cell[prev_base + 3] = bfloat16_t(c_new4.w);
    output_hidden[prev_base] = bfloat16_t(h_new4.x);
    output_hidden[prev_base + 1] = bfloat16_t(h_new4.y);
    output_hidden[prev_base + 2] = bfloat16_t(h_new4.z);
    output_hidden[prev_base + 3] = bfloat16_t(h_new4.w);
    return;
  }
  ...
  for (uint i = 0u; i < 4u && (h_base + i) < hidden_size; i++) {
    ...
    float f_g = fast_sigmoid(
        float(input_proj[b + hidden_size]) +
        float(hidden_proj[b + hidden_size]));
    float g_g = metal::fast::tanh(
        float(input_proj[b + 2u * hidden_size]) +
        float(hidden_proj[b + 2u * hidden_size]));
    float o_g = fast_sigmoid(
        float(input_proj[b + 3u * hidden_size]) +
        float(hidden_proj[b + 3u * hidden_size]));
