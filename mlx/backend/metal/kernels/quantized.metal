...
#pragma clang loop unroll(full)
      for (int j = 0; j < el_per_thread; j++) {
        x_block[simd_lid * el_per_thread + j] =
            x[i + simd_lid * el_per_thread + j];
      }
    }
    if (simd_lid == 0) {
#pragma clang loop unroll(full)
      for (int j = 0; j < groups_per_block; j++) {
        scales_block[simd_gid * groups_per_block + j] =
            scales[i / group_size + j];
      }
#pragma clang loop unroll(full)
      for (int j = 0; j < groups_per_block; j++) {
        biases_block[simd_gid * groups_per_block + j] =
            biases[i / group_size + j];
      }
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
...
#pragma clang loop unroll(full)
    ...
    scale = scales_block
        [simd_gid * groups_per_block + simd_lid * el_per_thread / group_size];
    bias = biases_block
        [simd_gid * groups_per_block + simd_lid * el_per_thread / group_size];
...
#pragma clang loop unroll(full)
    for (int k = 0; k < el_per_thread; k++) {
      result +=
          (scale * static_cast<U>(w_local & bitmask) + bias) * x_thread[k];
      w_local >>= bits;
    }
  }
...
[[kernel]] void qvm(
  ...
  for (int i = 0; i < in_vec_size; i += BM) {
    ...
    if (simd_lid < groups_per_block && group_in_bounds) {
      scales_block[simd_gid * groups_per_block + simd_lid] =
          scales[offset_gid * out_vec_size_g + simd_lid];
      biases_block[simd_gid * groups_per_block + simd_lid] =
          biases[offset_gid * out_vec_size_g + simd_lid];
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    scale = scales_block
        [simd_lid * groups_per_block + (simd_gid * el_per_int) / group_size];
    bias = biases_block
        [simd_lid * groups_per_block + (simd_gid * el_per_int) / group_size];
...
[[kernel]] void qmm_t(
  ...
  constexpr int groups_per_block =
      (BK / group_size > 0) ? (BK / group_size) : 1;
  constexpr int groups_per_simd = BN / (WM * WN);
  constexpr int w_els_per_thread =
      (BN * BK / el_per_int) / (SIMD_SIZE * WM * WN);

  // Instantiate the appropriate BlockMMA and Loader
  using mma_t =
      mlx::steel::BlockMMA<T, T, BM, BN, BK, WM, WN, false, true, BK, BK>;
  using loader_x_t =
      mlx::steel::BlockLoader<T, BM, BK, BK, 1, WM * WN * SIMD_SIZE, 1, 4>;
  ...
  for (int k = 0; k < K; k += BK) {
    ...
    if (simd_lid == 0) {
      threadgroup T* scales_block_local =
          scales_block + lidy * groups_per_block * groups_per_simd;
      threadgroup T* biases_block_local =
          biases_block + lidy * groups_per_block * groups_per_simd;
      const device T* scales_local =
          scales + lidy * groups_per_simd * K_g + k / group_size;
      const device T* biases_local =
          biases + lidy * groups_per_simd * K_g + k / group_size;
#pragma clang loop unroll(full)
    ...
    // Load the w tile
    {
      if (!aligned_N && num_outs < BN) {
        for (int wo = 0; wo < w_els_per_thread; wo++) {
          ...
          threadgroup T* Ws_local =
              Ws + offset_row * BK + offset_col * el_per_int;

          if (y_row + offset_row < N) {
            uint32_t wi = *w_local;
            T scale = scales_block
                [offset_row * groups_per_block +
                 offset_col / (group_size / el_per_int)];
            T bias = biases_block
                [offset_row * groups_per_block +
                 offset_col / (group_size / el_per_int)];
...
[[kernel]] void qmm_n(
  ...
  constexpr int el_per_int = 32 / bits;
  constexpr int groups_per_block =
      (BN / group_size > 0) ? (BN / group_size) : 1;
  constexpr int groups_per_simd = BK / (WM * WN);
  constexpr int w_els_per_thread =
      (BK * BN / el_per_int) / (SIMD_SIZE * WM * WN);

  // Instantiate the appropriate BlockMMA and Loader
  using mma_t =
      mlx::steel::BlockMMA<T, T, BM, BN, BK, WM, WN, false, false, BK, BN>;
  using loader_x_t =
      mlx::steel::BlockLoader<T, BM, BK, BK, 1, WM * WN * SIMD_SIZE, 1, 4>;
    ...
    // Load the w tile
    {
      if (num_k < BK) {
        for (int wo = 0; wo < w_els_per_thread; wo++) {
          ...
          threadgroup T* Ws_local =
              Ws + offset_row * BN + offset_col * el_per_int;
...
#define instantiate_qmv(name, itype, group_size, bits)  \
  ...
  qmv<itype, 32, 32, group_size, bits>(                 \
      const device uint32_t* w [[buffer(0)]],           \
...
#define instantiate_qvm(name, itype, group_size, bits)  \
  template [[host_name("qvm_" #name "_gs_" #group_size  \
                       "_b_" #bits)]] [[kernel]] void   \
  qvm<itype, 32, 32, group_size, bits>(                 \
  ...
  instantiate_qvm(float32, float, group_size, bits)    \
      instantiate_qvm(float16, half, group_size, bits) \
        ...
        instantiate_qvm_types(128, 2) instantiate_qvm_types(
            128,
            4) instantiate_qvm_types(128, 8) instantiate_qvm_types(64, 2)
            instantiate_qvm_types(64, 4) instantiate_qvm_types(
                64,
                8) instantiate_qvm_types(32, 2) instantiate_qvm_types(32, 4)
...
#define instantiate_qmm_t(name, itype, group_size, bits, aligned_N)  \
  ...
  qmm_t<itype, 32, 64, 32, group_size, bits, aligned_N>(             \
      const device itype* x [[buffer(0)]],                           \
      const device uint32_t* w [[buffer(1)]],                        \
      const device itype* scales [[buffer(2)]],                      \
      const device itype* biases [[buffer(3)]],                      \
                  ...
                  instantiate_qmm_t(float16, half, group_size, bits, true) \
                    ...
                    instantiate_qmm_t_types(128, 2) instantiate_qmm_t_types(
                        ...
                        instantiate_qmm_t_types(64, 2) instantiate_qmm_t_types(
                            ...
                            instantiate_qmm_t_types(
                                32,
                                2) instantiate_qmm_t_types(32, 4)
...
#define instantiate_qmm_n(name, itype, group_size, bits) \
  template [[host_name("qmm_n_" #name "_gs_" #group_size \
                       "_b_" #bits)]] [[kernel]] void    \
  qmm_n<itype, 32, 32, 64, group_size, bits>(            \
      const device itype* x [[buffer(0)]],               \
      const device uint32_t* w [[buffer(1)]],            \
      const device itype* scales [[buffer(2)]],          \
      const device itype* biases [[buffer(3)]],          \
