...
struct GEMVKernel {
  ...
  //
  // 1. A thread loads TN elements each from mat along TM contiguous rows and
  // the corresponding scalar from the vector
...
#define instantiate_gemv_c(name, itype, bm, bn, tm, tn)            \
  template [[host_name("gemv_" #name "_bm" #bm "_bn" #bn "_tm" #tm \
                       "_tn" #tn)]] [[kernel]] void                \
  gemv<itype, bm, bn, tm, tn>(                                     \
...
#define instantiate_gemv_t_c(name, itype, bm, bn, tm, tn)            \
  template [[host_name("gemv_t_" #name "_bm" #bm "_bn" #bn "_tm" #tm \
                       "_tn" #tn)]] [[kernel]] void                  \
  gemv_t<itype, bm, bn, tm, tn>(                                     \
      const device itype* mat [[buffer(0)]],                         \
      const device itype* vec [[buffer(1)]],                         \
      device itype* out [[buffer(2)]],                               \
      const constant int& in_vec_size [[buffer(3)]],                 \
