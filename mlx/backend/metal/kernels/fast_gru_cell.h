...
// Fused GRU cell: one kernel for gating. Inputs are pre-computed (e.g. by GEMM).
// Optimized: vectorized float4 path when 4 consecutive h fit, else scalar.
[[host_name("gru_cell_fused_float")]] [[kernel]] void gru_cell_fused_float(
  ...
  // GRU: n = tanh(x_n + r * h_proj_n), out = (1-z)*n + z*h_prev (h_proj_n = hidden_proj[2*H:])
  if (h_base + 4u <= hidden_size) {
    ...
    float4 x_z4 = *reinterpret_cast<const device float4*>(input_proj + base + hidden_size) +
                  *reinterpret_cast<const device float4*>(hidden_proj + base + hidden_size);
    float4 x_n4 = *reinterpret_cast<const device float4*>(input_proj + base + 2u * hidden_size);
    float4 h_proj_n4 = *reinterpret_cast<const device float4*>(hidden_proj + base + 2u * hidden_size);
    float4 h_prev4 = *reinterpret_cast<const device float4*>(hidden_prev + prev_base);

    float4 r4 = fast_sigmoid4(x_r4);
    float4 z4 = fast_sigmoid4(x_z4);
    float4 n4 = metal::fast::tanh(x_n4 + r4 * h_proj_n4);
    *reinterpret_cast<device float4*>(output + prev_base) = (1.0f - z4) * n4 + z4 * h_prev4;
    return;
  }
...
// BFloat16 path: vectorized float4 when possible, else scalar; math in float, bf16 on write.
[[host_name("gru_cell_fused_bfloat16")]] [[kernel]] void gru_cell_fused_bfloat16(
  ...
  if (h_base + 4u <= hidden_size) {
    float4 x_r4 = float4(input_proj[base], input_proj[base+1], input_proj[base+2], input_proj[base+3]) +
                  float4(hidden_proj[base], hidden_proj[base+1], hidden_proj[base+2], hidden_proj[base+3]);
    float4 x_z4 = float4(input_proj[base+hidden_size], input_proj[base+hidden_size+1], input_proj[base+hidden_size+2], input_proj[base+hidden_size+3]) +
    ...
    float4 x_n4 = float4(input_proj[base+2u*hidden_size], input_proj[base+2u*hidden_size+1], input_proj[base+2u*hidden_size+2], input_proj[base+2u*hidden_size+3]);
    float4 h_proj_n4 = float4(hidden_proj[base+2u*hidden_size], hidden_proj[base+2u*hidden_size+1], hidden_proj[base+2u*hidden_size+2], hidden_proj[base+2u*hidden_size+3]);
    float4 h_prev4 = float4(hidden_prev[prev_base], hidden_prev[prev_base+1], hidden_prev[prev_base+2], hidden_prev[prev_base+3]);
  ...
  for (uint i = 0u; i < 4u && (h_base + i) < hidden_size; i++) {
    ...
    float x_z = float(input_proj[b + hidden_size]) + float(hidden_proj[b + hidden_size]);
