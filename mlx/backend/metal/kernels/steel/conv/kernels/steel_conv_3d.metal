...
#define instantiate_implicit_conv_3d(                                          \
    ...
    filter_name,                                                               \
    small_filter)                                                              \
  template [[host_name("implicit_gemm_conv_3d_" #name "_bm" #bm "_bn" #bn     \
                       "_bk" #bk "_wm" #wm "_wn" #wn                          \
                       "_filter_" #filter_name)]] [[kernel]] void              \
  implicit_gemm_conv_3d<itype, bm, bn, bk, wm, wn, small_filter>(             \
...
#define instantiate_implicit_3d_filter(name, itype, bm, bn, bk, wm, wn)        \
...
#define instantiate_implicit_3d_blocks(name, itype)               \
    instantiate_implicit_3d_filter(name, itype, 32,  8, 16, 4, 1) \
    instantiate_implicit_3d_filter(name, itype, 64,  8, 16, 4, 1) \
    instantiate_implicit_3d_filter(name, itype, 32, 32, 16, 2, 2) \
    instantiate_implicit_3d_filter(name, itype, 32, 64, 16, 2, 2) \
    instantiate_implicit_3d_filter(name, itype, 64, 32, 16, 2, 2) \
    instantiate_implicit_3d_filter(name, itype, 64, 64, 16, 2, 2)
