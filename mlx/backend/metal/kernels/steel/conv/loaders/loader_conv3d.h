...
struct Conv3DInputBlockLoaderLargeFilter {
  ...
  METAL_FUNC void load_unsafe() const {
    ...
    for (short i = 0, is = 0; i < n_rows; ++i, is += TROWS) {
      // Find bounds
      int n = read_n[i];
      int id = read_id[i] + weight_d * params->kdil[0];
      int ih = read_ih[i] + weight_h * params->kdil[1];
      int iw = read_iw[i] + weight_w * params->kdil[2];
...
struct Conv3DWeightBlockLoader {
  ...
  /* Iteration helper */
  METAL_FUNC void next() {
    if (++weight_dhw < (params->wS[2] * params->wS[1] * params->wS[0])) {
      src += weight_step;
      return;
    }
    ...
    src +=
        BK - (params->wS[2] * params->wS[1] * params->wS[0] - 1) * weight_step;
  }
};
