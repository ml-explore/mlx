...
[[kernel, max_total_threads_per_threadgroup(WM* WN * 32)]] void addmm(
  ...
  if (MN_aligned) {
    ...
    mma_op.store_result(
        D, params->ldd, C, params->ldc, params->fdc, epilogue_op);
    return;
...
#define instantiate_gemm_bias_helper( \
    tname,                            \
    trans_a,                          \
    trans_b,                          \
    iname,                            \
  ...
  instantiate_gemm_transpose_helper(                               \
      ...
      instantiate_gemm_transpose_helper(                           \
          ...
          instantiate_gemm_transpose_helper(                       \
              ...
              instantiate_gemm_transpose_helper(                   \
                  iname, itype, oname, otype, 64, 32, 16, 2, 2)    \
                  instantiate_gemm_transpose_helper(               \
                      iname, itype, oname, otype, 32, 64, 16, 2, 2)
