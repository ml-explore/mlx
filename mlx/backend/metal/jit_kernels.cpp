...
MTL::ComputePipelineState* get_cholesky_kernel(
    ...
    const std::string& kernel_name,
    const array& out) {
  // kernel_name is "cholesky_{phase}_{type}", e.g. "cholesky_potf2_float32".
  // Build all the phases into one library per type.
  ...
  auto lib = d.get_library(lib_name, [&] {
    ...
    kernel_source += get_template_definition(
        "cholesky_potf2_" + tname, "cholesky_potf2", t_str);
    kernel_source += get_template_definition(
        "cholesky_trsm_" + tname, "cholesky_trsm", t_str);
    kernel_source += get_template_definition(
