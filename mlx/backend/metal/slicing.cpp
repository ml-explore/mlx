...
array compute_dynamic_offset(
  ...
  auto lib = d.get_library(lib_name, [dtype]() {
    // Clamp the start so the slice stays inside the operand, as XLA's
    // dynamic-slice does. Unclamped this offset addresses memory outside the
    // array entirely. The comparison is done in the index's own signedness so
    // that neither a negative signed start nor a large unsigned one clamps to
    // the wrong end of the axis.
    std::string in_range =
        "((uint64_t)indices[i] > hi ? (int64_t)max_starts[i] "
        ": (int64_t)indices[i])";
    std::string clamp = issubdtype(dtype, signedinteger)
        ? "indices[i] < 0 ? 0 : " + in_range
        : in_range;
    return fmt::format(
        R"(
        [[kernel]] void compute_dynamic_offset_{0}(
            ...
            constant const int* max_starts [[buffer(5)]],
            uint index [[thread_position_in_grid]]) {{
          int64_t acc = 0;
          for (int i = 0; i < n_axes; ++i) {{
            uint64_t hi = (uint64_t)max_starts[i];
            int64_t idx = {2};
  ...
  compute_encoder.set_vector_bytes(max_starts, 5);
