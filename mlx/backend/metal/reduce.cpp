...
void all_reduce_no_atomics_dispatch(
    const array& in,
    array& out,
    const std::string& op_name,
    MTL::ComputeCommandEncoder* compute_encoder,
  ...
  array intermediate(
      {static_cast<int>(intermediate_size)}, out.dtype(), nullptr, {});
...
void row_reduce_general_no_atomics_dispatch(
    const array& in,
    array& out,
    const std::string& op_name,
    const ReductionPlan& plan,
  ...
  if (non_row_reductions > 1) {
    std::vector<array> inputs = {};
    std::vector<int> intermediate_shape = {
        static_cast<int>(out.size()), static_cast<int>(non_row_reductions)};
    intermediate = std::make_unique<array>(
        intermediate_shape, out.dtype(), nullptr, inputs);
    intermediate->set_data(allocator::malloc_or_wait(intermediate->nbytes()));
...
void strided_reduce_general_no_atomics_dispatch(
    const array& in,
    array& out,
    const std::string& op_name,
    const ReductionPlan& plan,
  ...
  size_t thread_group_size = row_reduce_kernel->maxTotalThreadsPerThreadgroup();
  thread_group_size =
      std::min((reduction_size + n_reads - 1) / n_reads, thread_group_size);
...
void Reduce::eval_gpu(const std::vector<array>& inputs, array& out) {
  ...
  if (in.size() > 0) {
    ...
    if (plan.type == ContiguousAllReduce) {
      if (in.dtype() == uint64 || in.dtype() == int64) {
        all_reduce_no_atomics_dispatch(in, out, op_name, compute_encoder, d);
      } else {
    ...
    // the last dim.
      ...
      if (in.dtype() == uint64 || in.dtype() == int64) {
        row_reduce_general_no_atomics_dispatch(
    ...
    // strided reduce over these.
      ...
      if (in.dtype() == uint64 || in.dtype() == int64) {
        strided_reduce_general_no_atomics_dispatch(
            in, out, op_name, plan, axes_, compute_encoder, d);
      } else {
