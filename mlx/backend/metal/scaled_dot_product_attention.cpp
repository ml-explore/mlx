...
bool ScaledDotProductAttentionVJP::use_fallback(
  ...
  const int query_head_dim = q.shape(-1);
  const int value_head_dim = q.shape(-1); // Assume same as query for VJP
  const int query_seq_len = q.shape(2);
  const int n_q_heads = q.shape(1);

  // Compute GQA factor if n_kv_heads is provided
  const int gqa_factor =
      (n_kv_heads > 0) ? (n_q_heads / n_kv_heads) : 1;

  // Vector VJP now uses exp2() matching STEEL forward's log2 domain.
  // Note: D=256 exceeds Metal's 32KB threadgroup memory limit for vector VJP.
  // Note: The accumulate variant (used for half/bfloat16) does NOT support sinks.
  ...
  // For longer sequences or unsupported head dims, try STEEL VJP
  // STEEL VJP doesn't support masks/sinks - must use fallback
  if (has_mask || has_sinks) {
    // Check if we can use vector VJP (short seq + supported head dim)
    if (query_seq_len <= 8 && vector_supported_head_dim) {
      // For sinks with non-float32, must use fallback (accumulate doesn't support sinks)
      if (has_sinks && !is_float32) {
        return true;
      }
      return false; // Vector VJP supports masks (and sinks for float32)
    }
    return true; // Must use unfused attention
  }

  // STEEL VJP kernel constraints:
  // - Only supports D=64 for float16/bfloat16 due to threadgroup memory limits
  // - Float32 exceeds limits for all head dims
  const bool steel_supported_head_dim = !is_float32 && (query_head_dim == 64);

  if (!steel_supported_head_dim) {
    return true; // Must use fallback - unsupported config
  }

  // GQA-aware heuristic: STEEL VJP is slower than unfused for high GQA ratios
  // with longer sequences. Benchmarks show:
  // - gqa_factor >= 4 with seq_len >= 256: unfused is faster
  // - gqa_factor >= 4 with seq_len >= 128: marginal, prefer unfused for safety
  // The overhead comes from the sequential GQA loop in the dKV kernel.
  if (gqa_factor >= 4 && query_seq_len >= 128) {
    return true; // Unfused is faster for heavy GQA
  }

  return false; // Use STEEL VJP
}
...
// Grid: [NQ, H, B] - one threadgroup per query block per head per batch
void sdpa_steel_vjp_dq_dispatch(
  ...
  // Block dimensions - same as forward STEEL attention
  int wm = 4;
  int wn = 1;
  ...
  // Function constants (shared between dQ and dKV kernels)
  metal::MTLFCList func_consts = {
      {&align_Q, MTL::DataType::DataTypeBool, 200},
      {&align_K, MTL::DataType::DataTypeBool, 201},
      {&do_causal, MTL::DataType::DataTypeBool, 301}};

  // Build kernel name: attention_vjp_dq_<type>_<bq>_<bk>_<bd>
  std::string base_name;
  concatenate(
      base_name,
      "attention_vjp_dq_",
  ...
  AttnVJPParams params{
      ...
      /* int64_t dK_strides[3] = */ {0, 0, 0}, // Not used by dQ kernel
      /* int64_t dV_strides[3] = */ {0, 0, 0}, // Not used by dQ kernel
      /* int64_t LSE_strides[2] = */
      {logsumexp.strides(1), logsumexp.strides(2)}};

  // Set kernel arguments (dQ kernel uses buffer indices 0-7)
  compute_encoder.set_input_array(q, 0);
  compute_encoder.set_input_array(k, 1);
  compute_encoder.set_input_array(v, 2);
  compute_encoder.set_input_array(out, 3);
  ...
  // Grid dims: (NQ, H, B) - one threadgroup per query block per head per batch
  MTL::Size grid_dims = MTL::Size(NQ, H, B);
  // Group dims: (32, wm, wn) = (32, 4, 1) = 128 threads
  MTL::Size group_dims = MTL::Size(32, wm, wn);

  compute_encoder.dispatch_threadgroups(grid_dims, group_dims);
}

// Dispatch dKV kernel - computes dK and dV gradients directly (no accumulators)
// Grid: [NK, H, B] - one threadgroup per KV block per head per batch
void sdpa_steel_vjp_dkv_dispatch(
  ...
  // Build kernel name: attention_vjp_dkv_<type>_<bq>_<bk>_<bd>
  std::string base_name;
  concatenate(
      base_name,
      "attention_vjp_dkv_",
  ...
  AttnVJPParams params{
      ...
      /* int64_t dQ_strides[3] = */ {0, 0, 0}, // Not used by dKV kernel
  ...
  // Set kernel arguments (dKV kernel uses buffer indices 0-8)
  compute_encoder.set_input_array(q, 0);
  compute_encoder.set_input_array(k, 1);
  compute_encoder.set_input_array(v, 2);
  compute_encoder.set_input_array(out, 3);
  ...
  // Grid dims: (NK, num_kv_heads, B) - one threadgroup per KV block per KV head
  // per batch NOTE: This dispatches over KV heads (not query heads) to avoid
  // race conditions in GQA. The kernel loops over gqa_factor query heads
  // internally to accumulate dK/dV.
  int num_kv_heads = k.shape(1);
  MTL::Size grid_dims = MTL::Size(NK, num_kv_heads, B);
  // Group dims: (32, wm, wn) = (32, 4, 1) = 128 threads
  MTL::Size group_dims = MTL::Size(32, wm, wn);
...
void ScaledDotProductAttentionVJP::eval_gpu(
  ...
  const auto& q = copy_unless(q_copy_unless, q_pre);
  // Vector VJP requires row-contiguous K/V because the kernel uses input strides
  // for output pointer arithmetic, and dK/dV are always contiguous.
  ...
  // For short sequences (seq <= 8), the vector VJP kernel uses atomic adds
  // to accumulate dK and dV, so we must zero-initialize these arrays.
  // For longer sequences, the STEEL VJP kernel writes directly without atomics.
  const int query_head_dim = q.shape(-1);
  const int value_head_dim = v.shape(-1);
  ...
  if (use_vector_vjp) {
    ...
    // STEEL VJP kernel writes directly, no zero-initialization needed
    d_k.set_data(allocator::malloc(d_k.nbytes()));
    d_v.set_data(allocator::malloc(d_v.nbytes()));
  }
