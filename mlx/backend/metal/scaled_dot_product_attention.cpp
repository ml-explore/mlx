...
// Query rows processed serially by each simdgroup in the multi-row 2-pass
// vector kernel. Bounded by register pressure: larger head dims hold more
// per-lane accumulator state (see MAX_NQ in sdpa_vector.h).
inline int sdpa_vector_max_nq(int query_head_dim) {
  return query_head_dim <= 128 ? 4 : 2;
}

// Whether the multi-row 2-pass vector kernel supports the shape
// (8 < q_seq_len <= 16, threadgroup fits at the maximum rows per simdgroup).
inline bool sdpa_multirow_vector_supported(
    int query_head_dim,
    int value_head_dim,
  ...
  return supported_head_dim && query_sequence_length > 8 &&
      query_sequence_length <= 16 &&
      query_sequence_length <= key_sequence_length &&
      gqa_factor * min_rows_per_tg <= 32;
}

// Same as sdpa_vector_2pass but for 8 < q_seq_len <= 16, dispatching the
// multi-row first-pass kernel: each simdgroup owns nq consecutive query rows
// so the threadgroup (32 x gqa_factor x ceil(q_seq_len / nq)) stays within
// the 1024-thread limit where a row-per-simdgroup layout would not fit.
void sdpa_vector_2pass_multirow(
    const Stream& s,
    metal::Device& d,
    const array& q,
    const array& k,
  ...
  std::string kname;
  kname.reserve(64);
  kname += "sdpa_vector_2pass_1_multirow_";
  kname += get_type_string(q.dtype());
  kname += "_";
  kname += std::to_string(q.shape(-1));
  kname += "_";
  ...
  // Compute the necessary sizes. Rows per simdgroup: grow nq until the
  // threadgroup fits its simdgroups (32 lanes x gqa_factor x rows_per_tg
  // <= 1024 threads).
  int gqa_factor = q.shape(1) / k.shape(1);
  ...
  std::string base_hash_name = kname;
  base_hash_name +=
      has_mask ? (bool_mask ? "_boolmask" : "_floatmask") : "_nomask";
  base_hash_name += query_transposed ? "_qt" : "_qnt";
  base_hash_name += do_causal ? "_c" : "_nc";
  base_hash_name += has_sinks ? "_sinks_" : "_nosinks_";
  base_hash_name += std::to_string(blocks);
  std::string hash_name = base_hash_name + "_nq" + std::to_string(nq);

  // Get the kernel. High register pressure can lower the pipeline's
  // threadgroup limit; trade threadgroup rows for more rows per simdgroup
  // when that happens.
  auto kernel = d.get_kernel(kname, hash_name, func_consts);
  while (nq < max_nq &&
    ...
    hash_name = base_hash_name + "_nq" + std::to_string(nq);
...
bool ScaledDotProductAttention::use_fallback(
  ...
  const bool supports_sdpa_vector_multirow = sdpa_multirow_vector_supported(
      query_head_dim,
      value_head_dim,
...
void ScaledDotProductAttention::eval_gpu(
  ...
  // Multi-row vector mode: 8 < q_seq_len <= 16 (speculative-decoding verify
  // shapes). Route here when the keys are long enough that the vector
  // kernel's block-parallel reduction over the sequence beats the full
  // kernel's single query tile (the same threshold the decode routing above
  // uses for the 2-pass split), or when the full kernel does not support the
  // head dim and the alternative is the unfused fallback.
  else if (
      sdpa_multirow_vector_supported(
          q_pre.shape(-1),
          v_pre.shape(-1),
          q_pre.shape(2),
          k_pre.shape(2),
    ...
    sdpa_vector_2pass_multirow(
        s, d, q, k, v, o, scale_, do_causal_, mask, sinks);
  }
