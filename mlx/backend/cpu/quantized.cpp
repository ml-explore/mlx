...
void _qmm_t_simd_multi_col(
  ...
  for (int g = 0; g < groups; g++) {
    ...
    // Try ISA-specific batch-extract fast path (no-op stub returns 0 when
    // unavailable).
...

    {
      ...
      // (elem starts at 0 without Highway, or after batch-extracted elements)
...
void QQMatmul::eval_cpu(const std::vector<array>& inputs, array& out) {
  ...
  bool has_global_scales =
      mode_ == QuantizationMode::Nvfp4 && inputs.size() == 5;
  array global_scale_x(0.0f, float32);
  array global_scale_w(0.0f, float32);
  if (has_global_scales) {
    global_scale_x = ensure_row_contiguous(inputs[3]);
    global_scale_w = ensure_row_contiguous(inputs[4]);
  }
