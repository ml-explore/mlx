...
void eval(const std::vector<array>& outputs, bool retain_graph /* = false */) {
  ...
  for (auto& arr : outputs) {
    if (!arr.is_evaled() || (!arr.is_tracer() && arr.has_primitive())) {
      ...
      // with at the end.
      if (!arr.is_evaled() && deps.find(arr.id()) == deps.end()) {
...
//       throw std::invalid_argument(
//           "[vjp] Number of outputs with gradient does not match number of
//           cotangents.");
//     }
...
//       if (auto cotan_it = cotan_map.find(o.id()); cotan_it !=
//       cotan_map.end()) {
...
//   if (argnums.empty()) {
//     throw std::invalid_argument("[grad] Must specify at least one
//     argument.");
//   }
...
//     // appropriate floating point type op(int, floatXX) -> floatXX for most
//     ops auto [outputs, grads] = vjp(gfun, ginputs, {array(1)}); return
//     std::make_pair(outputs, grads);
//   };
// }
//
// namespace detail {
