...
void benchmark_all_reduce(const mx::Group& group) {
  ...
  std::vector<std::string> algorithms = {
      "default", "linear", "ring", "recursive_doubling", "tree"};

  std::cout << std::left << std::setw(20) << "Algorithm" << std::setw(15)
            << "Size (MB)" << std::setw(15) << "Latency (ms)" << std::setw(15)
            << "Bandwidth (GB/s)" << std::setw(10) << "Ops/sec" << std::endl;
  ...
  for (const auto& algo : algorithms) {
    for (int size : sizes) {
      ...
      double bandwidth_gbps =
          (size_bytes * group.size()) / (latency_ms / 1000.0) / (1e9);
      double ops_per_sec = 1000.0 / latency_ms;

      std::cout << std::left << std::setw(20) << algo << std::setw(15)
                << (size_bytes / (1024.0 * 1024.0)) << std::setw(15)
                << latency_ms << std::setw(15) << bandwidth_gbps
...
void benchmark_all_gather(const mx::Group& group) {
  ...
  std::vector<int> sizes = {1024, 65536, 262144, 1048576};

  std::vector<std::string> algorithms = {"default", "ring", "tree"};
  ...
  for (const auto& algo : algorithms) {
    for (int size : sizes) {
      ...
      auto fn = [&]() { return mx::distributed::all_gather_opt(x, algo); };
...
void benchmark_reduce_scatter(const mx::Group& group) {
  ...
  std::vector<int> sizes = {65536, 262144, 1048576};

  std::vector<std::string> algorithms = {"default", "ring"};
...
void benchmark_pipeline(const mx::Group& group) {
  ...
  std::vector<int> sizes = {1024, 65536};

  int num_stages = 4;

  std::cout << std::left << std::setw(20) << "Stages" << std::setw(15)
            << "Size (MB)" << std::setw(20) << "Latency (ms)" << std::setw(15)
            << "Ops/sec" << std::endl;
  ...
  for (int size : sizes) {
    ...
    std::cout << std::left << std::setw(20) << num_stages << std::setw(15)
              << (size_bytes / (1024.0 * 1024.0)) << std::setw(20) << latency_ms
              << static_cast<int>(ops_per_sec) << std::endl;
  }
}
...
void benchmark_bandwidth_scaling(const mx::Group& group) {
  ...
  std::vector<std::string> algorithms = {"ring", "tree"};

  std::cout << "Group size: " << group.size() << std::endl;
  std::cout << std::left << std::setw(20) << "Algorithm" << std::setw(15)
            << "Size (MB)" << std::setw(20) << "Bandwidth (GB/s)" << std::endl;
  ...
  for (const auto& algo : algorithms) {
    for (int size : sizes) {
      ...
      std::cout << std::left << std::setw(20) << algo << std::setw(15)
                << (size_bytes / (1024.0 * 1024.0)) << std::setw(20)
                << bandwidth_gbps << std::endl;
    }
  }
}
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--all" || arg == "-a") {
      ...
      std::cout << "  -m, --metrics   Print state-of-the-art metrics reference"
                << std::endl;
