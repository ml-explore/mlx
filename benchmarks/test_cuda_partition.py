...
def test_partition_basic():
    ...
    for size in [100, 500, 1000, 5000, 10000]:
        for k in [0, size // 4, size // 2, size - 1]:
            ...
            assert np.all(
                result_np[k + 1 :] >= actual_kth
            ), f"Failed for size={size}, k={k}"
...
def benchmark_partition():
    ...
    for size in sizes:
        ...
        print(
            f"  Size {size:>10}: partition={partition_time*1000:.2f}ms, sort={sort_time*1000:.2f}ms, speedup={speedup:.2f}x"
        )
