        ...
        for num_procs_test in sorted(set(num_processes_list)):
            ...
            try:
                ...
                if num_procs_test > 1 and world.rank() == 0:
                    ...
                    def run_single():
                        return mx.distributed.all_reduce_opt(x_single, op=operation, algo=algo)
                    
                    try:
                        time_single, _ = time_function(run_single, num_warmup=2, num_iters=5)
                        efficiency = calculate_scalability_efficiency(time_single, mean_time, num_procs_test)
                    except:
                        efficiency = 1.0
                else:
                    efficiency = 1.0
                ...
                if world is None or world.rank() == 0:
                    ...
                    print(f"    {num_procs_test:3d} processes, {size_mb:8.2f} MB total: "
...
def benchmark_pipeline_scaling(
    ...
    for num_procs_test in sorted(set(num_processes_list)):
        ...
        try:
            ...
            if world is None or world.rank() == 0:
                print(f"    {num_procs_test:3d} processes, {size_per_process/1024:.2f} KB/process: "
...
def run_weak_scaling_benchmark(
    ...
    for experiment_name, experiment in experiments.items():
        ...
        for key, results_list in results_dict.items():
            ...
            sorted_results = sorted(results_list, key=lambda x: (x.size_elements, x.num_processes))
        ...
        if results_list:
            single_proc_results = [r for r in results_list if r.num_processes == 1]
            multi_proc_results = [r for r in results_list if r.num_processes > 1]
            ...
            experiment.metrics["scalability_profile"] = {
                ...
                "efficiency_range": {
                    ...
                    "mean": sum(r.scalability_efficiency for r in multi_proc_results) / len(multi_proc_results)
                }
            }
    ...
    for num_cores in num_cores_list:
        ...
        try:
            ...
            if num_cores == 1:
                ...
                try:
                    time_single, _ = time_function(run_single_cpu, num_warmup=2, num_iters=5)
                    efficiency = calculate_scalability_efficiency(time_single, mean_time, num_cores)
                except:
            ...
            if world is None or world.rank() == 0:
                ...
                print(f"    {num_cores:3d} cores, {size_mb:8.2f} MB total: "
                ...
                metrics={
                    ...
                    "scalability_profile": {
                        ...
                        "efficiency_range": {
                            ...
                            "mean": sum(r.scalability_efficiency for r in results) / len(results)
                        }
                    }
                }
            )
        ...
        if "experiments" in data:
            for exp_name, experiment in data["experiments"].items():
                ...
                if "bandwidth_gbps" in exp_results[0]:
                    print(f"{'Processes':<12} {'Latency (ms)':>14} {'Bandwidth (GB/s)':>18} "
                          f"{'Efficiency':>12}")
                    print("-" * 60)
                    
                    for r in sorted(exp_results, key=lambda x: x.num_processes):
                        print(f"{r.num_processes:<12} {r.latency_ms:>14.3f} "
                              f"{r.bandwidth_gbps or 'N/A':>18} {r.scalability_efficiency:>12.2%}")
                else:
                    print(f"{'Processes':<12} {'Latency (ms)':>14} {'Throughput':>18} "
                          f"{'Efficiency':>12}")
                    print("-" * 60)
                    ...
                    if "scalability_profile" in metrics:
                        ...
                        if isinstance(profile, dict):
                            ...
                            if isinstance(eff_range, dict):
                                print(f"  Min efficiency: {eff_range.get('min', 0):.2%}")
                                print(f"  Max efficiency: {eff_range.get('max', 0):.2%}")
                                print(f"  Mean efficiency: {eff_range.get('mean', 0):.2%}")
                            else:
...
Note:
  ...
  - For CPU benchmarks: use single process with multiple threads
    ...
    parser.add_argument(
        "--gpu-scale",
        action="store_true",
        help="Run GPU scaling benchmarks"
    )
    
    parser.add_argument(
        "--cpu-scale",
        action="store_true",
        help="Run CPU scaling benchmarks"
    )
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Run full benchmark suite (GPU + CPU)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Minimum number of GPUs (default: 2)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=40,
        help="Maximum number of GPUs (default: 40)"
    )
    ...
    if args.full or (not args.gpu_scale and not args.cpu_scale and 
                     not args.weak_scaling and not args.strong_scaling):
