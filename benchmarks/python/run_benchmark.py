...
def main():
    ...
    try:
        ...
        print(
            f"⚠ Distributed initialization failed (expected in single-process mode): {e}"
        )
        num_processes = 1
    ...
    for result in all_results:
        ...
        if "size_elements" in result and result["num_processes"] == 1:
            ...
            if (
                "ring" in str(result)
                or "tree" in str(result)
                or "default" in str(result)
            ):
                # Check first few results to identify operation
                pass

        print(
            f"  {result['algorithm'] if 'algorithm' in result else 'pipeline'}: "
