...
TEST_CASE("thread_pool rapid max-thread parallel_for") {
  auto& pool = ThreadPool::instance();
  int nth = pool.max_threads();
  ...
  CHECK_EQ(total.load(), ITERS * nth);
}

TEST_CASE("thread_pool rapid varying max-thread counts") {
  auto& pool = ThreadPool::instance();
  int max_t = pool.max_threads();
  if (max_t < 2) {
    return;
  ...
  std::atomic<int> total{0};
  std::vector<std::atomic<int>> seen(max_t);
  ...
  for (int iter = 0; iter < ITERS; iter++) {
    for (int nth : counts) {
      ...
      for (int i = 0; i < max_t; i++) {
        int expected_seen = i < nth ? 1 : 0;
        int actual_seen = seen[i].load(std::memory_order_relaxed);
        if (actual_seen != expected_seen) {
          INFO("iter=", iter, " nth=", nth, " tid=", i);
          CHECK_EQ(actual_seen, expected_seen);
          return;
        }
      }
    }
  ...
  CHECK_EQ(total.load(), ITERS * expected);
}
...
TEST_CASE(
    "thread_pool concurrent max-thread parallel_for from multiple threads") {
  auto& pool = ThreadPool::instance();
  const int N_CALLERS = 4;
  const int ITERS = 250;
  int nth = pool.max_threads();
...
TEST_CASE("thread_pool parallel_for with exceptions does not deadlock") {
  // Current impl only propagates slot-0 exceptions, but it must not hang.
  auto& pool = ThreadPool::instance();
  int nth = std::min(pool.max_threads(), 4);
...
TEST_CASE("thread_pool parallel_for with varying thread counts") {
  ...
  // Alternate between different thread counts to exercise worker wakeup
  // across small and saturated dispatches.
