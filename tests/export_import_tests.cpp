...
TEST_CASE("test export callback includes dtype primitive state") {
  auto fun = [](std::vector<array> x) -> std::vector<array> {
    return {astype(x[0], int32)};
  };

  std::vector<StateT> astype_arguments;
  auto callback = [&astype_arguments](const ExportCallbackInput& input) {
    auto type_it = input.find("type");
    REQUIRE(type_it != input.end());
    if (!std::holds_alternative<std::string>(type_it->second)) {
      return;
    }
    if (std::get<std::string>(type_it->second) != "primitive") {
      return;
    }

    auto name_it = input.find("name");
    REQUIRE(name_it != input.end());
    if (!std::holds_alternative<std::string>(name_it->second)) {
      return;
    }
    if (std::get<std::string>(name_it->second) != "AsType") {
      return;
    }

    auto args_it = input.find("arguments");
    REQUIRE(args_it != input.end());
    REQUIRE(std::holds_alternative<std::vector<StateT>>(args_it->second));
    astype_arguments = std::get<std::vector<StateT>>(args_it->second);
  };

  export_function(callback, fun, {array({1.2f, 2.3f})});

  REQUIRE(astype_arguments.size() == 1);
  REQUIRE(std::holds_alternative<Dtype>(astype_arguments[0]));
  CHECK(std::get<Dtype>(astype_arguments[0]) == int32);
}
